...
def train_net(folder):
    num_samples = sum([len(files) for r, d, files in os.walk(folder)])
    num_train_samples = int(num_samples * 0.9)
    num_valid_samples = int(num_samples * 0.1)
    ...
    model = get_model()
    model.layers.pop()
    x = model.layers[-1].output     # add last layer
    x = Dense(256, activation="relu", kernel_regularizer=regularizers.l2(0.01))(x)
    x = Dense(len(classes), activation="softmax", name="fc1000")(x)
...
if __name__ == '__main__':
    """
    dataset_path: /Users/radekj/devroot/vmmr/datasets/ds5000
    /storage/plzen1/home/radekj/vmmr"

    """
    print(len(sys.argv))
    if len(sys.argv) < 2:
        print("Need param: python ds5000.py dataset_path")
        exit(1)
