...
def train_vgg(folder):
    ...
    train_gen = ImageDataGenerator(
            ...
            rotation_range=4,
            zoom_range=0.1)
    batches = train_gen.flow_from_directory(directory=TRAIN_DIR, target_size=SIZE,
                color_mode="rgb", batch_size=BATCH_SIZE, class_mode="categorical", shuffle=True, save_to_dir=save_aug)

    val_gen = ImageDataGenerator()
    val_batches = val_gen.flow_from_directory(directory=VALID_DIR, target_size=SIZE, color_mode="rgb",
                    batch_size=BATCH_SIZE, class_mode="categorical", shuffle=True)

    model = get_model()
    classes = list(iter(batches.class_indices))
    for layer in model.layers[:-6]:     # freeze first 10 layers
